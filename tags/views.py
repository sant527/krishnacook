...
def tag_list(request):
	...
	if form.is_valid():
		...
		return HttpResponseRedirect(reverse("tags:list"))  # add this so that the form will no longer contain the data.
...
@login_required
def tag_update(request, slug=None):
	instance = get_object_or_404(Tag, slug=slug)
	form = TagForm(request.POST or None, request.FILES or None,instance=instance)
	if form.is_valid():
		instance = form.save(commit=False)
		instance.save()
		messages.success(request, "<a href='#'>Item</a> Saved", extra_tags='html_safe')
		return HttpResponseRedirect(reverse("tags:list"))

	context = {
		"instance": instance,
		"form":form,
	}
	return render(request, "tags_update.html", context)


@login_required
def tag_confirm(request,slug=None):
	instance = get_object_or_404(Tag, slug=slug)
	context = {
		"instance": instance,
	}
	return render(request, "tags_confirm_delete.html", context)

@login_required
def tag_delete(request, slug=None):
	instance = get_object_or_404(Tag, slug=slug)
	instance.delete()
